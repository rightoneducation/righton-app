...
async function uploadAssessment(gql, classroomId, sessionId, parsed, type, sourceAssessmentId, docxQuestionByStored) {
...
/**
 * PARITY: mirrors buildDocxQuestionMap in src/cli/upload.ts.
 *
 * Maps the assessment's stored question numbering back to the document's, so
 * `docxQuestion` can be written onto the AssessmentQuestion rows.
 * `reconcileQuestionNumbers` asserts the correct-answer letter sequence matches
 * position-for-position and throws if it does not, rather than silently mapping
 * onto the wrong questions.
 */
function buildDocxQuestionMap(answerKey, questionMeta) {
  if (!answerKey?.length || !questionMeta?.length) return undefined;
  const qNumMap = reconcileQuestionNumbers(answerKey, questionMeta);
  const out = new Map();
...
async function runPipeline(input) {
  ...
  // The document numbers questions 1,3,5,7,9,11 (pilot files); the assessment
  // stores them resequenced (1..N). Keyed for the AssessmentQuestion rows so
  // `docxQuestion` is persisted alongside each stored question.
  const docxQuestionByStored = buildDocxQuestionMap(ingestData.answerKey, parsed.questionMeta);
  ...
  const ppqAssessment = await uploadAssessment(gql, classroomId, sessionId, ppqData, 'PPQ', undefined, docxQuestionByStored);
  ...
  const genFunctionName = `microcoachv2TeacherUploadGen-${AMPLIFY_ENV}`;
  const genPayload = {
    classroomId,
    sessionId,
