...
import { parsePpqTable, formatAnswerKey } from './util/parsePpqTable.mjs';
...
export const handler = async (event) => {
  ...
  // Parse the answer key deterministically. A malformed table is fatal: without
  // it upload cannot reconcile the document's question numbering to the stored one.
  const answerKey = parsePpqTable(ppqText);
  const answerKeyBlock = formatAnswerKey(answerKey);
  console.log(
    `[microcoachIngestPPQ] parsed ${answerKey.length} questions: ` +
    answerKey.map((q) => `Q${q.docxQuestion}=${q.correctAnswer}`).join(' ')
  );
...
## Answer Key
The document's answer key, parsed: question number as printed, correct option letter,
and the standard each question assesses.
...
**How to identify misconceptions from the questions:**
- Read each question in the document body together with its correct answer from the key
- Each incorrect answer choice is designed to catch a specific error pattern; work out the cognitive mistake that would lead a student to it
- Group related error patterns across questions that share the same root misconception
- Rank misconceptions by how many questions share the same error pattern (priority "1" = most prevalent)
...
`.trim();
  ...
  try {
    ...
    // The answer key travels with the response so upload can reconcile the
    // document's question numbering to the stored one.
    structured.answerKey = answerKey;
