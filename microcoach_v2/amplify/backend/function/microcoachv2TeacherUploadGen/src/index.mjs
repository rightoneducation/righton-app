...
async function runGeneratePipeline(gql, classroom, sessionId) {
  ...
  const lsResults = await Promise.all(
    allCcss.map(async (ccss) => {
      const raw = await invokeLambda(`microcoachv2GetLearningScience-${AMPLIFY_ENV}`, {
  ...
  try {
    const raw = await invokeLambda(`microcoachv2NextStepOption-${AMPLIFY_ENV}`, {
      input: {
        planStructures: true,
  ...
  if (EVALUATORS_ENABLED && evalTexts.length > 0) {
    ...
    try {
      evalResults = await invokeLambda(`microcoachv2InitialEvaluator-${AMPLIFY_ENV}`, {
