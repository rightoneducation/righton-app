 ...
 *
 * TypeScript twin of `microcoachv2ScoresCalc/src/util/ccssCode.mjs` — the pipeline
 * needs this in both runtimes and a Lambda cannot import from `seed/`. Keep the two
 * in step; the behaviour is covered by that function's `node --test` suite.
 *
 ...
 * same standard, so a lookup keyed on the literal string finds nothing.
 *
 *   HSA-REI.B.3  →  rei.3      A.REI.3  →  rei.3
 *   8.EE.C.8     →  8.ee.8     8.EE.8   →  8.ee.8
 *
 * Lossy on purpose: it assumes standard numbers do not repeat within a domain,
 * which holds across CCSS math. Grade numbers survive — the filter drops single
 * *letters*, not single characters — so `7.EE.4` and `8.EE.4` stay distinct. Because
 * it is lossy, `matchStandard` tries the literal string first.
 */

export interface StandardMatch<T> {
  standard: T | null;
  matchedBy: 'exact' | 'canonical' | null;
}

/** Whitespace-stripped, lowercased — the exact-match key. */
export const normalizeCode = (c: unknown): string => String(c ?? '').replace(/\s/g, '').toLowerCase();

/** The spelling-independent key. See the traces above. */
export function canonicalCcss(code: unknown): string {
 ...
 * second, so today's behaviour is unchanged wherever the spellings already agree.
 */
export function matchStandard<T extends { code?: string }>(
  code: unknown,
  standards: T[] | undefined,
): StandardMatch<T> {
