...
describe("Testing ModelHelper", () => {
  const createGameSession = (teamTotalCount: number) => {
    ...
    for (let i = 0; i < teamTotalCount; i++) {
      const team = TeamHelper.team(`Team ${i + 1}`, gameSession.id);
      const teamMember = TeamMemberHelper.teamMember(team.id);
      gameSession.questions!.forEach((question) => {
        const answer = TeamAnswerHelper.teamAnswer(
          teamMember.id,
          question.id,
  ...
  const addTrickAnswer = (
    ...
    return TeamAnswerHelper.teamAnswer(
      teamMember!.id,
      question.id,
  ...
  test("Calculate trick answer score for one question game session", () => {
    ...
    question.choices!.forEach((choice) => {
      const score = ModelHelper.calculateBasicModeWrongAnswerScore(
        gameSession,
        choice.text,
        question.id
      );
  ...
  test("Calculate total score for one question game session", () => {
    ...
    gameSession.teams!.forEach((team, idx) => {
      ...
      team.score =
        team.score +
        ModelHelper.calculateBasicModeScoreForQuestion(
          gameSession,
          question,
          team
        );
  ...
  test("Calculate total score for multiple question game session", () => {
    ...
    gameSession.teams!.forEach((team, idx) => {
      gameSession.questions!.forEach((question) => {
        ...
        //user is entering trick answer
        team.teamMembers![0]?.answers!.push(
          addTrickAnswer(team, idx, question)
        );
        //CHOOSE_TRICKIEST_ANSWER
        gameSession.currentState = GameSessionState.PHASE_2_RESULTS;
        team.score =
        ...
        else gameSession.currentQuestionIndex!--;
      });
