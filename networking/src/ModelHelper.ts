...
import { IChoice, IQuestion } from "./Models";
...
export abstract class ModelHelper {
  ...
  static getBasicTeamMemberAnswersToQuestionId(
    team: ITeam,
    questionId: string
  ): Array<ITeamAnswer | null> | null {
    if (isNullOrUndefined(team.teamMembers) || team.teamMembers.length == 0) {
    ...
    if (
      isNullOrUndefined(teamMember.answers) ||
  ...
  static getSelectedTrickAnswer(
    team: ITeam,
    questionId: string
  ): ITeamAnswer | null {
    if (isNullOrUndefined(team.teamMembers) || team.teamMembers.length !== 1) {
    ...
    const trickAnswer = teamMember!.answers?.find((answer) => {
      ...
      return answer.questionId === questionId && answer.isTrickAnswer;
    });

    return trickAnswer ?? null;
  }

  static calculateBasicModeWrongAnswerScore(
    ...
    const totalNoChosenAnswer = gameSession.teams.reduce(
      (previousVal: number, team: ITeam) => {
        if (
          isNullOrUndefined(team.teamMembers) ||
  ...
  static calculateBasicModeScoreForQuestion(
    gameSession: IGameSession,
    question: IQuestion,
    team: ITeam
  ) {
    ...
    const answers = this.getBasicTeamMemberAnswersToQuestionId(
      team,
      question.id
    );
    if (isNullOrUndefined(answers) || answers.length === 0) {
      return 0;
    }
    ...
    let submittedTrickAnswer = answers.find(
      (answer) =>
        answer?.isTrickAnswer && answer.questionId === currentQuestion.id
    );

    if (submittedTrickAnswer) {
      return ModelHelper.calculateBasicModeWrongAnswerScore(
        gameSession,
        submittedTrickAnswer.text ?? "",
        currentQuestion.id
      );
    } else if (
      answers.find(
  ...
  static findTeamMemberInTeam(
    ...
    return (
      team.teamMembers.find(
        (member) => !isNullOrUndefined(member) && member.id === teamMemberId
      ) ?? null
    );
  }
}
