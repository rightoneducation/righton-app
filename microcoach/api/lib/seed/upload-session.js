...
if (!classroomId || !classroomKey || !sessionLabel) {
    console.error('Usage: npx ts-node src/seed/upload-session.ts --classroomId <id> --classroom <key> --session <label>\n' +
        'Example: npx ts-node src/seed/upload-session.ts --classroomId abc-123 --classroom Classroom1 --session Session2');
    process.exit(1);
}
const classroomConfig = seedData_1.CLASSROOMS.find((c) => c.key === classroomKey);
if (!classroomConfig) {
...
function parseExcelFile(filePath) {
    ...
    if (rows.length < 8)
        throw new Error(`Excel file ${filePath} has fewer than 8 rows`);
    ...
    const classRow = (_h = rows[3]) !== null && _h !== void 0 ? _h : [];
    ...
    for (const col of questionColumns) {
        const val = String((_k = keyRow[col]) !== null && _k !== void 0 ? _k : '').trim().toUpperCase();
    ...
    for (let r = studentStartRow; r < rows.length; r++) {
        const row = rows[r];
        const name = String((_l = row[nameCol]) !== null && _l !== void 0 ? _l : '').trim();
        if (!name)
            continue;
        if (name.toLowerCase().includes('total') || name.toLowerCase().includes('average'))
            continue;
        const externalId = String((_m = row[externalIdCol]) !== null && _m !== void 0 ? _m : '').trim();
        const rawScore = parseFloat(String((_o = row[scoreCol]) !== null && _o !== void 0 ? _o : '0'));
        ...
        const questionResponses = questionColumns.map((col, i) => {
            ...
            if (isAssessmentMatrix) {
                if (rawResponse === '')
                    return { questionNumber: questionNumbers[i], response: correctAnswer, isCorrect: true, pointsEarned: 1 };
...
async function uploadAssessment(sessionId, parsed, type, sourceAssessmentId) {
...
async function uploadStudentResponses(label, assessmentId, students, studentMap) {
    const start = Date.now();
