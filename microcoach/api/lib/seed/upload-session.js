...
function parseExcelFile(filePath) {
    ...
    for (let col = 1; col < headerRow.length; col++) {
        if (String((_g = headerRow[col]) !== null && _g !== void 0 ? _g : '').trim().match(/^q?(\d+)$/i))
            allNumericCols.push(col);
    }
    const confColByQNumber = {};
    for (let col = 1; col < headerRow.length; col++) {
        const cMatch = String((_h = headerRow[col]) !== null && _h !== void 0 ? _h : '').trim().match(/^Q?(\d+)_Conf$/i);
        if (cMatch)
...
async function main() {
    ...
    console.log(`  PostPPQ file : ${(_a = sessionConfig.postPpqFile) !== null && _a !== void 0 ? _a : '(none — skipped)'}\n`);
    // Parse PPQ
