...
async function putTeacherItemInDynamoDB(apiName, account, items, onSuccess, onError) {
  try {
    const path = '/TeacherID';
    const myInit = {
      body: {
...
async function getTeacherItemFromDynamoDB(apiName, account, onSuccess, onError) {
  try {
    const path = `/TeacherID/object/${account.TeacherID}`;
    const response = await API.get(apiName, path);
    onSuccess(response);
...
async function deleteTeacherItemInDynamoDB(apiName, account, onSuccess, onError) {
  try {
    const path = `/TeacherID/object/${account.TeacherID}`;
    const response = await API.del(apiName, path);
    onSuccess(response);
