...
import { IQuestionTemplate, PublicPrivateType, SortDirection, SortType, GradeTarget } from '@righton/networking';
...
export default function useExploreQuestionsStateManager(): UseExploreQuestionsStateManagerProps {
  ...
  const handleChooseGrades = (grades: GradeTarget[]) => {
    ...
    setIsLoading(true);
    setNextToken(null);
    apiClients?.centralDataManager?.searchForQuestionTemplates(PublicPrivateType.PUBLIC, null, null, searchTerms, sort.direction ?? SortDirection.ASC, sort.field, [...grades]).then((response) => {
      setIsLoading(false);
  ...
  const handleSortChange = (newSort: { field: SortType; direction: SortDirection | null }) => {
    ...
    setIsLoading(true);
    setNextToken(null);
    apiClients?.centralDataManager?.searchForQuestionTemplates(PublicPrivateType.PUBLIC, null, null, searchTerms, newSort.direction ?? SortDirection.ASC, newSort.field, selectedGrades).then((response) => {
      setIsLoading(false);
      setSearchedQuestions(response.questions);
    })
  };
  ...
  const debouncedSearch = useCallback(  // eslint-disable-line
    debounce((search: string, sortDirection: SortDirection, gradeTargets: GradeTarget[], sortType: SortType) => {
      setNextToken(null);
      apiClients?.centralDataManager?.searchForQuestionTemplates(PublicPrivateType.PUBLIC, null, null, search, sortDirection, sortType, gradeTargets).then((response) => {
        setIsLoading(false);
        setSearchedQuestions(response.questions);
      })
    }, debounceInterval),
  ...
  const loadMoreQuestions = () => {
    if (nextToken && !isLoadingInfiniteScroll) {
      ...
      apiClients.questionTemplate.listQuestionTemplates(PublicPrivateType.PUBLIC, 12, nextToken, null, null, [])
        .then(response => {
          if (response){
