...
export const getGameElements = (openTab: LibraryTabEnum, isSearchResults: boolean, centralData: ICentralDataState) => {
  switch (openTab){
    case LibraryTabEnum.FAVORITES:
      if (isSearchResults)
        return centralData.searchedGames.filter((game) => centralData.userProfile?.favoriteGameTemplateIds?.includes(game.id));
...
export const getQuestionElements = (openTab: LibraryTabEnum, isSearchResults: boolean, centralData: ICentralDataState) => {
  switch (openTab){
    case LibraryTabEnum.FAVORITES:
      if (isSearchResults)
        return centralData.searchedQuestions.filter((question) => centralData.userProfile?.favoriteQuestionTemplateIds?.includes(question.id));
