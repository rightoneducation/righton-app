...
export default function LauncherGradesMenu({
  ...
  const handleGradesChange = (grade: string) => {
    const target = gradeMap[grade as keyof typeof gradeMap];
    if (!selectedGrades.includes(target)) {
      onGradesChange([...selectedGrades, target]);
    } else {
      onGradesChange(selectedGrades.filter((g) => g !== target));
    }
  };
