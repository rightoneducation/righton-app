...
import { Typography, RadioGroup, Box, Fade, styled, useTheme, InputAdornment, Grid, Stack } from '@mui/material';
    ...
    return (
      <BaseCardStyled elevation={6} isHighlight={isHighlight} isCardComplete={draftQuestion.questionCard.isCardComplete}>
        ...
        <ContentContainerStyled screenSize={screenSize}>
          ...
          sx={{ input: { "&::placeholder": { fontSize: '1.5rem', fontWeight: 700 }}}}
          InputProps={{
            startAdornment: 
          ...
          >
            <Typography variant="h2">{draftQuestion.questionCard.title}</Typography>
          </TextContainerStyled>
            <TextContainerStyled 
              multiline 
              variant="outlined" 
        ...
        <Grid container direction="row" alignItems="center">
            <Grid item md={6}>
                <Stack direction={screenSize === ScreenSize.SMALL ? 'column': 'row'} spacing={1} alignItems="center">
                    <SelectPhaseButton phaseNumber={1} screenSize={screenSize} />
                    <SelectPhaseButton phaseNumber={2} screenSize={screenSize} />
                </Stack>
                
            </Grid>
            <Grid item md={6} sx={{ display: 'flex', gap: '16px', alignItems: 'center', justifyContent: 'center' }}>
