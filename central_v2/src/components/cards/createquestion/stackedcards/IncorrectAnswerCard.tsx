...
const AnswerCard = styled(Paper, {
  shouldForwardProp: (prop) =>
    ...
    prop !== 'isCardClicked' &&
    prop !== 'isAIEnabled' &&
    prop !== 'isAIExplanationGenerated' &&
    prop !== 'isTopCard' &&
    prop !== 'isAIRegenEnabled' &&
    prop !== 'isClone',
})<StyledCardProps>(
  ({
    theme,
    ...
    paddingBottom:
      isAIEnabled && isAIExplanationGenerated && isTopCard && !isAIRegenEnabled
        ? '70px'
...
export default function IncorrectAnswerCard({
  ...
  const debouncedCardChanges = useMemo(
    () =>
      debounce(
        (
          debounceCardData: IncorrectCard,
          debouncedDraftQuestion: CentralQuestionTemplateInput,
          debounceCompleteAnswers: IncorrectCard[],
          debounceIncompleteAnswers: IncorrectCard[],
        ) => {
          setIsCardClicked(false);
          handleIncorrectCardStackUpdate(
            debounceCardData,
            debouncedDraftQuestion,
            debounceCompleteAnswers,
            debounceIncompleteAnswers,
          );
        },
        1000,
      ),
  ...
  const handleLocalAnswerChange = (value: string) => {
    ...
    debouncedCardChanges(
      { ...cardData, answer: value },
      draftQuestion,
      completeAnswers,
      incompleteAnswers,
    );
  ...
  const handleAIExplanationChange = (value: string, isRegen?: boolean) => {
    ...
    if (isRegen) setIsAIRegenEnabled(false);
  ...
  return (
    <Box style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      <AnswerCard
        ...
        isAIExplanationGenerated={isAIGeneratedLocal}
        isAIRegenEnabled={isAIRegenEnabled}
        isTopCard={isTopCard ?? false}
        isClone={isClone}
      ...
      >
        ...
        <TextContainerStyled
          ...
          sx={{
            ...
            '& .MuiInputBase-input': {
              color: '#47366C',
              opacity:
                isCardSubmitted && cardData.explanation.length === 0 ? 1 : 0.5,
              '&::placeholder': {
                color:
          ...
          <AnimatePresence>
            {isAIGeneratedLocal && isAIEnabled && isTopCard && (
              <>
                ...
                >
                  <img src={aiMonster} alt="AI Monster" />
                </motion.div>
              </>
            )}
          </AnimatePresence>
        ...
        {isTopCard &&
          ...
          (cardData.explanation.length === 0 ||
            cardData.answer.length === 0) && <ErrorBox />}
      </AnswerCard>
      {isTopCard && (
        <Box
          style={{
        ...
        >
          <CentralButton
            ...
            onClick={() =>
              handleNextCardButtonClick && handleNextCardButtonClick(cardData)
            }
          />
        </Box>
      )}
