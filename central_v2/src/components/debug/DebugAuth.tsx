...
export default function DebugAuth() {
  ...
  const handleLogin = async () => {
    setIsLoading(true);
    if (!checked) {
      const response = await apiClients.auth.awsSignIn(
        'debuguser',
        'debugpassword123',
      );
  ...
  const getIcon = () => {
    if (isLoading) return <CircularProgress color="inherit" size="30px" />;
    if (checked)
      return (
        <CheckCircleIcon
          style={{ color: '#000', height: '30px', width: '30px' }}
        />
      );
    return (
      <ClearIcon style={{ color: '#000', height: '30px', width: '30px' }} />
    );
  };
