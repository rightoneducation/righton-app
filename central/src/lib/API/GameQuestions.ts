...
export const deleteGameQuestions = async (apiClients: IAPIClients, id: string): Promise<IGameQuestion | null> => {
  try {
    const gameQuestions = await apiClients.gameQuestions.deleteGameQuestions(id);
    return gameQuestions;
  } catch (e) {
    console.log(e);
  }
