...
export const fetchGames = async (
  ...
  const result = (await API.graphql(graphqlOperation(listGames))) as {
    data: ListGamesQuery | null | undefined;
  };
...
export const createGame = async (game: any, questionIDSet: any) => {
  const newGame = (await API.graphql(graphqlOperation(CG, { game }))) as {
    data: any;
  };
  ...
  for (let i = 0; i < questionIDSet.length; i++) {
    result = (await API.graphql(
      graphqlOperation(CGQ, {
        gameQuestion: { gameId: newGameId, questionId: questionIDSet[i] },
      })
    )) as { data: any };
  }
...
export const updateGame = async (game: any) => {
  const editGame = (await API.graphql(graphqlOperation(UG, { game }))) as {
    data: any;
  };
...
export const cloneGame = async (game: any) => {
  ...
  for (let i = 0; i < questions.length; i++) {
    result = (await API.graphql(
      graphqlOperation(CGQ, {
        gameQuestion: { gameId: newGameId, questionId: questions[i].id },
      })
    )) as { data: any };
  }
  return newGame?.data?.createGame;
...
export const deleteGames = async (id: number) => {
  const result = (await API.graphql(graphqlOperation(DG, { id }))) as {
    data: DeleteGameMutation | null | undefined;
  };
...
export const deleteQuestions = async (id: number) => {
  const result = (await API.graphql(
    graphqlOperation(deleteQuestion, { id })
  )) as { data: DeleteQuestionMutation | null | undefined };
  return result?.data?.deleteQuestion || [];
};

export const getGame = async (id: any) => {
  const result = (await API.graphql(graphqlOperation(GG, { id }))) as {
    data: any;
  };
  return result.data;
};
