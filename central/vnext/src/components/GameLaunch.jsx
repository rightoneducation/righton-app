...
function GameForm({
  ...
  const match = useMatch('/games/:gameId/question/:questionIndex');
  ...
  const changeQuestionIndex = (currentIndex, newIndex) => {
    ...
    saveGame(newGame).then(() => history(`/games/${game.id}`));
    setAnchorEl(null);
    setActiveIndex(null);
  };

  const addQuestion = () =>
    history(`/gamemaker/${game.id}/createquestion`, {
      state: { game },
    });
    ...
    <>
        ...
        {isUserAuth && (
          <Grid>
            ...
            >
              ...
              <MenuItem onClick={cloneHandler()}>Clone</MenuItem>
      ...
      <Grid container>
        <Grid item xs={12} md={4} className={classes.leftComponent}>
          ...
          {game.imageUrl ? (
            <img className={classes.gameImage} src={game.imageUrl} alt="" />
          ) : (
            ...
            return (
              <Grid key={question.id} item xs={12} md={6}>
                <Card
                  ...
                  onClick={() => {
                    history(`/games/${game.id}/questions/${question.id}`, {
                      state: {
                        game,
                        question,
                      },
                    ...
                    {isUserAuth && (
                      <Grid item xs={3}>
                        ...
                        >
                          ...
                          <MenuItem
                            onClick={() => {
                              deleteQuestion(question.id, game).then(() =>
                                history(`/games/${game.id}`),
                              );
                              setAnchorEl(null);
                              setActiveIndex(null);
                            }}
