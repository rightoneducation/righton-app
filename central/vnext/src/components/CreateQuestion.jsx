...
export default function QuestionForm({
  ...
  const { originalQuestion, game } = location.state || null;
  ...
  const [question, setQuestion] = useState(() => {
    return (
      originalQuestion ?? {
        ...
        imageUrl: '',
        choices: [
          { text: '', reason: '', isAnswer: true },
          { text: '', reason: '', isAnswer: false },
          { text: '', reason: '', isAnswer: false },
          { text: '', reason: '', isAnswer: false },
        ],
        instructions: [],
  ...
  const isNullOrEmpty = (str) => !str || str.length < 1;

  // When the correct answer is changed/update this function handles that change
  const onChangeMaker = useCallback(
    (field) =>
      ({ currentTarget }) => {
        setQuestion({
          ...question,
          [field]: handleStringInput(currentTarget.value),
        });
      },
  ...
  const onChoiceTextChangeMaker = useCallback(
    (choiceIndex) =>
      ({ currentTarget }) => {
  ...
  const onChoiceReasonChangeMaker = useCallback(
    (choiceIndex) =>
      ({ currentTarget }) => {
        const newChoices = [...question.choices];
  ...
  const onSelectMaker = useCallback(
    (field) =>
      ({ target }) => {
        setQuestion({ ...question, [field]: target.value });
      },
    [question, setQuestion],
  );
  ...
  const handleSaveQuestion = async () => {
    ...
    if (
      ...
      question.instructions.filter((step) => !isNullOrEmpty(step)).length < 1
    ) {
      window.alert(
        'Please provide at least one step for the correct answer explanation',
      );
      return;
    }
    ...
    questionToSend.choices = question.choices;
    questionToSend.instructions = questionToSend.instructions.filter(
      (step) => step !== '',
    );

    let numToRemove;
    if (isNullOrUndefined(originalQuestion)) {
      numToRemove = 0;
    } else {
      numToRemove = 1;
    }
    game.questions.splice(questionIndex, numToRemove, questionToSend);
    updateGame(game);
    history(-1);
  };
        ...
        <Grid item container xs={7} sm={8}>
          <Grid item container xs={12}>
            <Typography gutterBottom variant="h4" component="h1">
              {isNullOrUndefined(originalQuestion) ? 'Edit' : 'Create'} Question
            </Typography>
          </Grid>
          ...
          <Grid item container xs={8}>
            ...
            <TextField
              id="image-url"
              onChange={onChangeMaker('imageUrl')}
              fullWidth
          ...
          <Grid item container justifyContent="center" xs={8} sm={4}>
            {question.imageUrl ? (
              <img src={question.imageUrl} alt="" width="60%" />
            ) : (
              <img
          ...
          <Grid item container xs={12}>
            <Typography
              ...
              variant="h4"
              component="h1"
              style={{ marginRight: 30, display: 'inline' }}
            >
              Enter CCSS
            </Typography>
          ...
          >
            <Button
              className={classes.addGameButton}
              variant="contained"
              color="primary"
