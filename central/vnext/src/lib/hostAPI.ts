...
export const removeTeam = async (teamId: number, gameSession: any) => {
  const updatedGameSession = {
    ...gameSession,
    teams: {
      items: gameSession.teams.items.filter(
        (team: { id: number }) => team.id !== teamId
      ),
    },
  };
...
export const changeGameStatus = async (currentState: any, gameSession: any) => {
  ...
  const updatedGameSession = {
    ...
    currentState:
      gameStatuses[
        gameStatuses.indexOf(currentState) !== gameStatuses.length - 1
          ? gameStatuses.indexOf(currentState) + 1
          : 0
      ],
  };
  return updatedGameSession;
