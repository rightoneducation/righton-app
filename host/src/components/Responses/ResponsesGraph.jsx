...
const ResponsesGraph = ({ inputArray }) => {
  ...
  const data = inputArray.map((word) => ({
    word: word[0],
    count: word[1],
  }));
    ...
    <div className={classes.container}>
      <div className={classes.titleContainer}>
        <Typography className={classes.title}>
          Responses aggregated by common words
        </Typography>
      </div>
      <div ref={graphRef} >
        <VictoryChart
        ...
        >
          ...
          <VictoryBar
            ...
            labels={({ datum }) => `${datum.word}`}
            barWidth={({ datum }) => datum.count !== 0 ? barThickness : barThicknessZero}
            dataComponent={
              <CustomBar
