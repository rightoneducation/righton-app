...
export default function Timer({
  ...
  const calculateCurrentTime = () => {
    if (gameSession) {
      ...
      // will need to change this. just bring in the allotted time math from gipContainer
      const allottedTime = 180;
  ...
  const [currentTimeMilli, setCurrentTimeMilli] = useState(calculateCurrentTime() * 1000); // millisecond updates to smooth out progress bar
  const currentTime = Math.trunc(currentTimeMilli / 1000);
  const progress = (currentTimeMilli / (totalTime * 1000)) * 100;
