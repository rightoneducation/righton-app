...
export default function LeaderboardSelector({teamName, teamAvatar, teamScore} : LeaderboardSelectorProps) {
  ...
  return(
    <LeaderboardSelectorContainer sx={{background: monsterMap[teamAvatar].gradient}}>
      <MonsterAvatar src={monsterMap[teamAvatar].monster} alt="avatar" />
      <Typography variant="h1" sx={{textShadow: '0px 2px 8px rgba(0, 0, 0, 0.7)', zIndex: 1, paddingLeft: `${theme.sizing.extraLargePadding}px`}}> {teamNameFormatted} </Typography>
      <ScoreBox>
        <Typography variant="h1" sx={{paddingLeft: `${theme.sizing.extraSmallPadding}px`, paddingRight: `${theme.sizing.extraSmallPadding}px` }}> {teamScore} </Typography>
      </ScoreBox>
